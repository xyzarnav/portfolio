...
const Navbar = ({ activePage, setActivePage }) => {
	const handleNavClick = (page) => {
		...
		// Access fullpageApi if on mobile
		if (window.innerWidth < 768 && window.fullpage_api) {
			// Move to the section based on page index
			const sectionIndex = ["about", "resume", "portfolio", "contact"].indexOf(
				page
			);
			if (sectionIndex >= 0) {
